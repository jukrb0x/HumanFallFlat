...
namespace TMPro
{
	public class TMP_Text : MaskableGraphic
	{
		...
		protected float GetPreferredWidth()
		{
			...
			m_recursiveCount = 0;
			Vector2 vector = CalculatePreferredValues(defaultFontSize, marginSize, ignoreTextAutoSizing: true);
			float x = vector.x;
		...
		protected float GetPreferredWidth(Vector2 margin)
		{
			...
			m_maxFontSize = m_fontSizeMax;
			m_recursiveCount = 0;
			Vector2 vector = CalculatePreferredValues(defaultFontSize, margin, ignoreTextAutoSizing: true);
			return vector.x;
		}
		...
		protected virtual Vector2 CalculatePreferredValues(float defaultFontSize, Vector2 marginSize, bool ignoreTextAutoSizing)
		{
			...
			for (int i = 0; m_char_buffer[i] != 0; i++)
			{
				...
				if (num4 == 9 || (!char.IsWhiteSpace((char)num4) && num4 != 8203) || m_textElementType == TMP_TextElementType.Sprite)
				{
					...
					num10 = m_xAdvance + m_cached_TextElement.xAdvance * ((num4 == 173) ? num14 : num2);
		...
		protected virtual void DrawTextHighlight(Vector3 start, Vector3 end, ref int index, Color32 highlightColor)
		{
			...
			Color32[] colors = m_textInfo.meshInfo[0].colors32;
			highlightColor.a = ((m_htmlColor.a >= highlightColor.a) ? highlightColor.a : m_htmlColor.a);
		...
		protected bool ValidateHtmlTag(int[] chars, int startIndex, out int endIndex)
		{
			...
			case 41311:
			{
				...
				if (!MaterialReferenceManager.TryGetFontAsset(valueHashCode3, out TMP_FontAsset fontAsset))
				{
			...
			case 154158:
			{
				MaterialReference materialReference3 = m_materialReferenceStack.Remove();
