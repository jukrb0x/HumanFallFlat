	...
	public class TextMeshProUGUI : TMP_Text, ILayoutElement
	{
		...
		protected override void GenerateTextMesh()
		{
			...
			case TextAlignmentOptions.TopGeoAligned:
				a = ((m_overflowMode == TextOverflowModes.Page) ? (rectTransformCorners[1] + new Vector3(margin.x, 0f - m_textInfo.pageInfo[num10].ascender - margin.y, 0f)) : (rectTransformCorners[1] + new Vector3(margin.x, 0f - m_maxAscender - margin.y, 0f)));
				break;
			...
			case TextAlignmentOptions.CenterGeoAligned:
				a = ((m_overflowMode == TextOverflowModes.Page) ? ((rectTransformCorners[0] + rectTransformCorners[1]) / 2f + new Vector3(margin.x, 0f - (m_textInfo.pageInfo[num10].ascender + margin.y + m_textInfo.pageInfo[num10].descender - margin.w) / 2f, 0f)) : ((rectTransformCorners[0] + rectTransformCorners[1]) / 2f + new Vector3(margin.x, 0f - (m_maxAscender + margin.y + num15 - margin.w) / 2f, 0f)));
				break;
			...
			case TextAlignmentOptions.BottomGeoAligned:
				a = ((m_overflowMode == TextOverflowModes.Page) ? (rectTransformCorners[0] + new Vector3(margin.x, 0f - m_textInfo.pageInfo[num10].descender + margin.w, 0f)) : (rectTransformCorners[0] + new Vector3(margin.x, 0f - num15 + margin.w, 0f)));
				break;
			...
			float num41 = m_previousLossyScaleY = lossyScale.y;
			...
			for (int j = 0; j < m_characterCount; j++)
			{
				...
				case TextAlignmentOptions.CaplineFlush:
				{
					...
					if ((!char.IsControl(character3) && lineNumber < m_lineNumber) || flag13 || tMP_LineInfo.maxAdvance > tMP_LineInfo.width)
					{
						...
						float num53 = (num52 <= 0) ? 1f : m_wordWrappingRatios;
						if (num52 < 1)
						{
							num52 = 1;
						}
						if (character2 == '\t' || char.IsSeparator(character2))
						{
							if (!m_isRightToLeft)
							{
								b2 += new Vector3(num50 * (1f - num53) / (float)num52, 0f, 0f);
							}
							else
							{
								b2 -= new Vector3(num50 * (1f - num53) / (float)num52, 0f, 0f);
						...
						{
							b2 += new Vector3(num50 * num53 / (float)num51, 0f, 0f);
						}
						else
						{
							b2 -= new Vector3(num50 * num53 / (float)num51, 0f, 0f);
				...
				if (isVisible)
				{
					...
					case TMP_TextElementType.Character:
					{
						...
						case TextureMappingOptions.Line:
							...
							{
								characterInfo[j].vertex_BL.uv2.x = (characterInfo[j].vertex_BL.position.x + b2.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num54;
								characterInfo[j].vertex_TL.uv2.x = (characterInfo[j].vertex_TL.position.x + b2.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num54;
								characterInfo[j].vertex_TR.uv2.x = (characterInfo[j].vertex_TR.position.x + b2.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num54;
								characterInfo[j].vertex_BR.uv2.x = (characterInfo[j].vertex_BR.position.x + b2.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num54;
							}
							break;
				...
				if ((m_textInfo.characterInfo[j].style & FontStyles.Underline) == FontStyles.Underline)
				{
					...
					if (flag && m_characterCount == 1)
					{
						...
						DrawUnderlineMesh(start, zero, ref index, num43, num44, num45, num42, color);
						num45 = 0f;
				...
				if (flag15)
				{
					...
					if (!flag2 && flag16 && j <= tMP_LineInfo.lastVisibleCharacterIndex && character2 != '\n' && character2 != '\r' && (j != tMP_LineInfo.lastVisibleCharacterIndex || !char.IsSeparator(character2)))
					{
						...
						start2 = new Vector3(m_textInfo.characterInfo[j].bottomLeft.x, m_textInfo.characterInfo[j].baseLine + strikethrough * num49, 0f);
					...
					{
						flag2 = false;
						zero2 = new Vector3(m_textInfo.characterInfo[j].topRight.x, m_textInfo.characterInfo[j].baseLine + strikethrough * num49, 0f);
						DrawUnderlineMesh(start2, zero2, ref index, num49, num49, num49, num42, underlineColor);
					}
					else if (flag2 && j == tMP_LineInfo.lastCharacterIndex)
					{
						...
						else
						{
							...
							zero2 = new Vector3(m_textInfo.characterInfo[lastVisibleCharacterIndex2].topRight.x, m_textInfo.characterInfo[lastVisibleCharacterIndex2].baseLine + strikethrough * num49, 0f);
						}
						flag2 = false;
						DrawUnderlineMesh(start2, zero2, ref index, num49, num49, num49, num42, underlineColor);
					}
					...
					{
						flag2 = false;
						zero2 = new Vector3(m_textInfo.characterInfo[j - 1].topRight.x, m_textInfo.characterInfo[j - 1].baseLine + strikethrough * num49, 0f);
						DrawUnderlineMesh(start2, zero2, ref index, num49, num49, num49, num42, underlineColor);
					}
				}
				else if (flag2)
