...
namespace HumanAPI
{
	public class ServoMotor : Node, INetBehavior
	{
		...
		private void FixedUpdate()
		{
			...
			if (joint != null)
			{
				...
				if (mode == ServoMode.Position)
				{
					...
					speed = Mathf.MoveTowards(speed, Mathf.Sign(num4 - num) * joint.maxSpeed, joint.maxAcceleration * Time.fixedDeltaTime);
		...
		public void ResetMotorState()
		{
			if (showDebug)
			{
				Debug.Log(base.name + " Resetting the state of reached end ");
			}
			reachedEnd = 0f;
		}
